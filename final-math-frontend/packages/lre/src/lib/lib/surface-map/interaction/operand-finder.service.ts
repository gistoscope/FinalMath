...
import { IOperandFinder } from '../interfaces';
import { SurfaceMap, SurfaceNode } from '../types';

@injectable()
export class OperandFinderService implements IOperandFinder {
  /**
      ...
      if (operatorNode && operatorNode.bbox) {
        ...
        if (!leftNode) {
          ...
          if (leftCandidates.length > 0) {
            // Sort by closest to op (largest right)
        ...
        if (!rightNode) {
          ...
          if (rightCandidates.length > 0) {
            // Sort by closest to op (smallest left)
