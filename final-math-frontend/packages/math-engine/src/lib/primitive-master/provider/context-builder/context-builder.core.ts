...
import type { NodeContext } from './context-builder.type';
...
export class NodeContextBuilder {
  public buildContext(params: {
    ...
    const effectiveNodeId =
      rawNodeId.length > 0 ? rawNodeId : ((ast as any).id ?? 'root');

    const effectiveClick: ClickTarget = { ...click, nodeId: effectiveNodeId };

    console.log(
      `[NodeContext] Building for Click: ${JSON.stringify(effectiveClick)}`,
    );
    ...
    if (!node) {
      console.error(
        `[NodeContext] FAILED to find node: ${effectiveClick.nodeId}`,
      );
    ...
    if (!operatorLatex) {
      ...
      if (parent) {
        ...
        if ((parent as any).op) {
          ...
          (parent as any).ops &&
          typeof click.operatorIndex === 'number'
        ) {
    ...
    function denomKey(den: any): string | null {
      ...
      if (typeof den === 'number' || typeof den === 'bigint')
        return String(den);
      if (typeof den === 'object') {
        // Common AST shapes
        if (
          typeof (den as any).value === 'string' ||
          typeof (den as any).value === 'number'
        )
    ...
    if (node.type === 'binaryOp' && (node as any).left && (node as any).right) {
      ...
      if (leftDen && rightDen) {
        ...
        guards['denominators-equal'] =
          leftKey !== null && rightKey !== null && leftKey === rightKey;
        guards['denominators-different'] = !guards['denominators-equal'];
        console.log(
          `[NodeContext] Computed denominators from node itself: leftKey=${leftKey}, rightKey=${rightKey}, equal=${guards['denominators-equal']}, different=${guards['denominators-different']}`,
        );
      }
    }
    // Fallback: check parent if node itself didn't have fraction operands
      ...
      if (leftDen && rightDen) {
        ...
        guards['denominators-different'] = !guards['denominators-equal'];
        console.log(
          `[NodeContext] Computed denominators from parent: leftKey=${leftKey}, rightKey=${rightKey}, equal=${guards['denominators-equal']}, different=${guards['denominators-different']}`,
        );
      }
    }
    ...
    if (
      ...
      console.log(
        `[NodeContext] Setting is-decimal=true for value: ${(node as any).value}`,
      );
    }

    if (actionNode && (actionNode as any).left)
      leftType = this.normalizeType((actionNode as any).left.type);
    if (actionNode && (actionNode as any).right)
      rightType = this.normalizeType((actionNode as any).right.type);
