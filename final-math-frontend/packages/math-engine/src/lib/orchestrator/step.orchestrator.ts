...
import { EngineRunner } from '../engine/index.js';
...
// Utils
import type { HistoryProvider } from '../interfaces';
...
export class StepOrchestrator {
  ...
  async updateHistory(history: StepHistory) {
    return this.historyProvider.updateHistory(
  ...
  async runStep(
    ...
    console.log({ req });
    if (!ctx) {
      ctx = this.contextGenerator.generateContext();
    }
    const {
      courseId,
      selectionPath,
    ...
    if (preferredPrimitiveId === 'P.INT_TO_FRAC') {
      const result = this.intToFracExecutor.execute(
        ast,
        selectionPath || 'root',
      );
      if (result.ok) {
    ...
    if (preferredPrimitiveId === 'P.ONE_TO_TARGET_DENOM') {
      const result = this.oneToTargetDenomExecutor.execute(
        ast,
        selectionPath || 'root',
      );
      if (result.ok) {
    ...
    if (ctx.primitiveMaster && ast) {
      ...
      console.log({ ast, selectionPath, operatorIndex, preferredPrimitiveId });
      ...
      if (clickTarget) {
        ...
        const v5Result = await this.v5OutcomeHandler.handle(
          v5Outcome,
          req,
          clickTarget,
          history,
   ...
   */
  async undoLastStep(): Promise<string | null> {
    ...
    await this.updateHistory(newHistory);
  ...
  async generateHint(
    ...
    if (decision.decision.status === 'chosen') {
      ...
      const chosenCandidate = legacyResult.candidates.find(
        (c) => c.id === chosenId,
      );
