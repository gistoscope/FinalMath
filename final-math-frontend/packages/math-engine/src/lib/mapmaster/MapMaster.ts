...
import { MapMasterSelectionNormalizer } from './providers/selection-normalizer/index.js';
...
export class MapMaster {
  ...
  generate(input: MapMasterInput): MapMasterResult {
    ...
    const resolvedSelectionPath = normalized
      ? normalized.anchorPath.join('.')
      : undefined;
