...
import { AstNode, parseExpression } from "../src/mapmaster/ast";
...
function findMatch(node: AstNode, matcher: GenericPatternMatcher): boolean {
  ...
  if (node.type === "binaryOp") {
    return (
      findMatch((node as any).left, matcher) ||
      findMatch((node as any).right, matcher)
    );
  }
...
async function runAudit() {
  ...
  const matrix: TestScenario[] = JSON.parse(
