...
export function loadInvariantRegistryFromFile(config) {
    ...
    try {
        ...
        throw new Error(`Failed to read invariant course file at "${absolutePath}": ${message}`);
    }
    let parsed;
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`Failed to parse invariant course file at "${absolutePath}" as JSON: ${message}`);
    }
    const validation = validateInvariantModel(parsed);
    if (!validation.ok || !validation.model) {
        const details = validation.issues.length === 0
            ? "unknown validation error"
            : validation.issues
                .map((issue) => `${issue.code} at ${issue.path}: ${issue.message}`)
                .join("; ");
        throw new Error(`Invariant course model in "${absolutePath}" is invalid: ${details}`);
    }
