 ...
 */
function resolveBackendRootFromHere() {
 ...
 */
export function loadInvariantModelFromFile(fullPath) {
    let raw;
    try {
        ...
        throw new Error(`[InvariantCourseLoader] Failed to read course file at "${fullPath}": ${message}`);
    }
    let parsed;
    try {
        ...
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`[InvariantCourseLoader] Failed to parse JSON from "${fullPath}": ${message}`);
    }
    ...
    if (!validation.ok || !validation.model) {
        if (validation.issues.length > 0) {
            const details = validation.issues
                ...
                .join("\n");
            console.error(`[InvariantCourseLoader] Invalid invariant model in "${fullPath}":\n${details}`);
        }
        throw new Error(`[InvariantCourseLoader] Course file "${fullPath}" is invalid; see log output for details.`);
    }
 ...
 */
export function loadInvariantRegistryFromFile(config) {
    if (!config || typeof config.path !== "string" || config.path.trim() === "") {
        throw new Error("[InvariantCourseLoader] config.path must be a non-empty string");
    }
 ...
 */
export function loadAllCoursesFromDir(config) {
    if (!config || typeof config.path !== "string" || config.path.trim() === "") {
        throw new Error("[InvariantCourseLoader] config.path must be a non-empty string");
    }
    const backendRoot = resolveBackendRootFromHere();
