 ...
 * and request context.
 */
import { findPatternForRule, findDomainForRule } from './mapmaster.invariants.registry';
/**
 ...
 */
export class DefaultInvariantRegistryAdapter {
    constructor(registry, astHelpers) {
     ...
     */
    getInvariantRulesForRequest(request, _windowRootPath, windowRootNode) {
        ...
        const augmentedRules = activeRules.map(rule => {
     ...
     */
    matchesPattern(rule, windowRootNode) {
     ...
     */
    haveSameDenominator(left, right) {
        if (!this.astHelpers) {
     ...
     */
    haveSameDenominatorFallback(left, right) {
        const leftDenom = left.denominator;
        const rightDenom = right.denominator;
