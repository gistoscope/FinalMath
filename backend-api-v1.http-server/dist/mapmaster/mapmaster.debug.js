 ...
 */
export function mapMasterDebug(input) {
    ...
    const ruleProvider = new DefaultMapMasterRuleProvider(selectionNormalizer, astHelpers, invariantRegistryAdapter, {
    ...
    if (normalized) {
        ...
        if (windowRootNode) {
            ...
            pipeline.window = {
                ...
                operation: windowRootNode.type === 'binaryOp' ? windowRootNode.op : undefined,
                nodeIds: [windowRootPath.join('.')]
            };
            // Step 3: Query Invariants
            const invariantRules = invariantRegistryAdapter.getInvariantRulesForRequest(input, windowRootPath, windowRootNode);
