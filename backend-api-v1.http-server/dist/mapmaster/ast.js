    ...
    function parsePrimary() {
        ...
        if (token.type === "COMMAND") {
            ...
            if (token.value === "frac") {
                ...
                consume(); // }
                if (peek()?.type !== "LBRACE")
                    throw new Error("Expected { for denominator");
                consume(); // {
                ...
                if (isSimple) {
                    ...
                    if (num.type === "integer")
                        simpleNum = num.value;
                    else if (num.type === "variable")
                        simpleNum = num.name;
                    if (den.type === "integer")
                        simpleDen = den.value;
                    else if (den.type === "variable")
                        simpleDen = den.name;
                    return {
                        type: "fraction",
                        numerator: simpleNum,
...
function preprocessMixedNumbers(tokens) {
    const result = [];
    let i = 0;
...
export function getNodeByOperatorIndex(ast, targetIndex) {
    let currentIndex = 0;
    let found;
...
function shouldParen(parentOp, child, isRightChild) {
