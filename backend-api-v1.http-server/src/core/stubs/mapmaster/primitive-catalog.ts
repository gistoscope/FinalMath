...
import { AstNode, getNodeAt } from "./ast";
...
export const primitiveCatalog: PrimitiveCatalogEntry[] = [
  ...
  },
  {
    ...
    lhsKind: ["int", "one", "zero"],
    rhsKind: ["int", "one", "zero"],
    primitiveId: "P.INT_DIV_EXACT", // Was P.INT_DIV_EXACT
 ...
 */
export function selectPrimitivesForClick(
  ctx: ClickContext
  ...
  return primitiveCatalog.filter((entry) => {
    ...
    // 4. Extra Conditions
    if (
      entry.sameDenominator !== undefined &&
      entry.sameDenominator !== ctx.sameDenominator
    ) {
      return false;
    }
...
function matchKind(
  entryKind: OperandKind | OperandKind[],
  ctxKind: OperandKind
): boolean {
