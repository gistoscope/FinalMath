...
export class PrimitiveMaster {
  ...
  public async resolvePrimitive(params: {
    ...
    const ast =
      params.ast || (await this.deps.parseLatexToAst(params.expressionLatex));
    if (!ast) {
    ...
    const normalizedClick =
      params.click.kind === "number" ||
      params.click.kind === "operator" ||
      params.preferredPrimitiveId
        ? {
   ...
   */
  public async match(
    request: PrimitiveMasterRequest
  ): Promise<PrimitiveMasterResult> {
    try {
