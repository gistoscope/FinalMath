...
export class PrimitiveRunner {
  static run(req: EngineStepExecutionRequest): EngineStepExecutionResult {
    ...
    try {
      ...
      if (resultPattern && bindings && !forceLegacy) {
        ...
        newAst = this.generateResultFromPattern(
          ast,
          targetPath,
          resultPattern,
          bindings
  ...
  private static generateResultFromPattern(
    root: AstNode,
    path: string,
  ...
  private static applyPrimitive(
    root: AstNode,
    target: AstNode | undefined | any,
    id: PrimitiveId,
    path: string
  ): AstNode | undefined {
    switch (id) {
      ...
      case "P.FRAC_MUL_UNIT":
        ...
        if (target?.type === "binaryOp" && target.op === "*") {
          if (
            target.left.type === "fraction" &&
            target.right.type === "fraction"
          ) {
      ...
      case "P.NEG_BEFORE_NUMBER":
        ...
        if (
          target?.type === "binaryOp" &&
          target.op === "-" &&
          target.left.type === "integer" &&
          target.left.value === "0"
        ) {
  ...
  private static runIntegerOp(
    ...
    if (
      ...
      (id === "P.INT_DIV_TO_INT" || id === "P.INT_DIV_TO_FRAC")
    ) {
    ...
    switch (id) {
      case "P.INT_ADD":
        resValue = this.runNumericBinaryOp(
          "+",
          target.left.value,
          target.right.value
        );
      ...
      case "P.INT_DIV_TO_INT": // Legacy support if needed, but EXACT is the V5 standard
        if (target.left.value.includes(".") || target.right.value.includes("."))
          return undefined;
        try {
      ...
      case "P.DECIMAL_DIV":
        resValue = this.runNumericDivision(
          target.left.value,
          target.right.value
        );
        break;
  ...
  private static parseNumericToScaledInt(value: string): {
    n: bigint;
    scale: number;
  } {
  ...
  private static runNumericBinaryOp(
    op: "+" | "-" | "*",
    leftValue: string,
    rightValue: string
  ): string {
  ...
  private static runDecimalToFraction(
    root: AstNode,
    target: AstNode | undefined,
    path: string
  ): AstNode | undefined {
  ...
  private static runNumericDivision(
    leftValue: string,
    rightValue: string
  ): string | null {
    const { n: n1, scale: s1 } = this.parseNumericToScaledInt(leftValue);
  ...
  private static runFractionOp(
    ...
    if (target?.type !== "binaryOp") {
      console.log(
        `[V5-RUNNER-FRAC] target is not binaryOp: path=${path}, targetType=${
    ...
    console.log(
      `[V5-RUNNER-FRAC] target.left.type=${
        target.left?.type
      } leftParts=${JSON.stringify(leftParts)}`
    );
    console.log(
      `[V5-RUNNER-FRAC] target.right.type=${
        target.right?.type
      } rightParts=${JSON.stringify(rightParts)}`
    );

    if (!leftParts || !rightParts) {
      console.log(
        `[V5-RUNNER-FRAC] FAIL: getFractionParts returned null for left or right`
      );
      return undefined;
    }
    ...
    switch (id) {
      case "P.FRAC_ADD_SAME_DEN":
        ...
        console.log(
          `[V5-RUNNER-START] primitiveId=${id} operator=${
            ctx.operatorLatex
          } left=fraction(${n1}/${d1}) right=fraction(${n2}/${d2}) guards=${JSON.stringify(
            ctx.guards
          )}`
        );
  ...
  private static getFractionParts(
  ...
  private static runDistributeNeg(
    root: AstNode,
    target: AstNode | undefined,
    id: string,
    path: string
  ...
  private static runDoubleNeg(
    ...
    if (target?.type === "binaryOp" && target.op === "-") {
      if (
        target.right.type === "integer" &&
        target.right.value.startsWith("-")
      ) {
  ...
  private static runLiftFraction(
    root: AstNode,
    target: AstNode | undefined,
    path: string,
    side: "left" | "right"
  ...
  private static runOneToTargetDenom(
    root: AstNode,
    target: AstNode,
    path: string
  ): AstNode | undefined {
