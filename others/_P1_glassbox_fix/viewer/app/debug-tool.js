...
async function handleForceIntToFrac() {
    ...
    // Resolve selectionPath from UI
    let selectionPath = null;
    if (selType === 'AstPath') {
        selectionPath = els.astPath.value || null;
    } else {
        // For operator index mode, we'll use "root" as default for integers
        selectionPath = "root";
    }
...
<div style="color: #fbbf24;">⏳ Sending request...</div>
    ...
    try {
        ...
        const outputLatex = json.output?.expressionLatex || json.newExpressionLatex || 'N/A';
        const chosenPrimitiveId = json.debugInfo?.chosenPrimitiveId || json.chosenPrimitiveId || 'N/A';
...
<div style="color: #f472b6;">Chosen Primitive: <span style="color: #fbbf24;">${chosenPrimitiveId}</span></div>
</div>
...
${status === 'step-applied'
                ...
                : status === 'no-candidates'
                    ? '⚠️ <span style="color: #f59e0b; font-weight: bold;">NO CANDIDATES: Backend does not support P.INT_TO_FRAC for this target</span>'
                    : '❌ <span style="color: #ef4444; font-weight: bold;">ERROR: ' + (json.error || json.message || 'Unknown error') + '</span>'
            }
</div>
        `.trim();
