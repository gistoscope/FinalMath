...
function applyP1Action(surfaceNodeId, astNodeId, cycleIndex) {
  const primitive = integerCycleState.primitives[cycleIndex];
  if (!primitive) {
    console.warn("[P1] No primitive for cycleIndex", cycleIndex);
    return;
  }

  console.log(`[P1] Applying action: primitive=${primitive.id}, surfaceNodeId=${surfaceNodeId}, astNodeId=${astNodeId}`);

  // Create a synthetic double-click event to trigger engine
  // The engine-adapter checks for clickCount === 2 OR dblclick type
  const syntheticEvent = {
    type: "click",
    timestamp: Date.now(),
    latex: typeof currentLatex === "string" ? currentLatex : "",
    surfaceNodeId: surfaceNodeId,
    surfaceNodeKind: "Num",
    astNodeId: astNodeId,
    click: {
      button: "left",
      clickCount: 2, // Mark as double-click for engine-adapter
      modifiers: { shift: false, alt: false, ctrl: false, meta: false }
    }
  };

  // Publish to FileBus so engine-adapter picks it up
  if (typeof fileBus !== "undefined" && fileBus.publishClientEvent) {
    fileBus.publishClientEvent(syntheticEvent);
  }
}
