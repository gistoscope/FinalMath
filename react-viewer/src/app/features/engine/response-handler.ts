// features/engine/response-handler.ts
// Engine response handling

import { getCurrentLatex, setCurrentLatex } from "../../core/state";
...
let _renderFormula: (() => void) | null = null;
let _buildAndShowMap: (() => void) | null = null;
let _clearSelection: ((reason: string) => void) | null = null;
 ...
 */
export function handleApplyChoice(
  primitiveId: string,
  targetPath: string,
  latex: string,
) {
  applyChoice(primitiveId, targetPath, latex, (newLatex: string) => {
 ...
 */
export function handleEngineResponse(res: any, debugState: any) {
