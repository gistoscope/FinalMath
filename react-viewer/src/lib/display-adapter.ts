...
export interface ClientEventClick {
  button: string;
  clickCount: number;
  modifiers: {
    shift: boolean;
...
export class DisplayAdapter {
   ...
   */
  emitHover(node: SurfaceNode | null, e: React.PointerEvent | PointerEvent) {
   ...
   * @param {PointerEvent} e
   */
  emitClick(node: SurfaceNode | null, e: React.PointerEvent | PointerEvent) {
  ...
  _baseEvent(
    ...
    e: React.PointerEvent | PointerEvent | null
  ): ClientEvent {
    ...
    const astOperatorIndex =
      node &&
      typeof node.astOperatorIndex === "number" &&
      Number.isFinite(node.astOperatorIndex)
    ...
    return {
      ...
      surfaceNodeText:
        node && node.latexFragment ? String(node.latexFragment) : undefined, // NEW: operator symbol ("+", "-", etc.)
   ...
   * @param {any} evt
   */
  _attachSelection(evt: ClientEvent) {
