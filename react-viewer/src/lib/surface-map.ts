...
export interface SurfaceNodeMap {
  root: SurfaceNode;
  atoms: SurfaceNode[];
...
interface Segment {
  type: "num" | "op" | "var" | "other";
  text: string;
}

function toRelativeBox(el: HTMLElement | Element, containerBox: DOMRect): BBox {
...
function isStructuralClass(className: string): boolean {
...
function isStructural(classes: string[]): boolean {
  return classes.some(isStructuralClass);
}

const OP_CHARS = "+-−*/:⋅·×÷";

function hasOperatorChar(text: string): boolean {
  const t = text || "";
 ...
 */
function segmentMixedContent(text: string): Segment[] {
...
export function buildSurfaceNodeMap(
  ...
  const byElement = new Map<HTMLElement, SurfaceNode>();
  let idCounter = 0;
  ...
  function traverse(
    ...
    const mixedNumAndOp = hasDigitChar(text) && hasOperatorChar(text);
    ...
    if (mixedNumAndOp) {
      ...
      segments.forEach((segment, idx) => {
        ...
        let kind: string, role: string, idPrefix: string, atomic: boolean;
        if (segment.type === "num") {
          kind = "Num";
          role = "operand";
          idPrefix = "num";
        ...
        const syntheticNode: SurfaceNode = {
          id: nextId(idPrefix),
          kind,
          role,
          bbox: segmentBBox,
    ...
    Array.from(element.children || []).forEach((child) =>
      traverse(child as HTMLElement, node)
    );
  }

  if (bases && bases.length) {
    bases.forEach((b) => traverse(b as HTMLElement, rootNode));
  }
 ...
 */
function normalizeOpSymbol(ch: string): string {
  const s = (ch || "").trim();
...
export function surfaceMapToSerializable(map: SurfaceNodeMap): any {
  function toPlain(node: SurfaceNode): any {
    return {
      id: node.id,
      kind: node.kind,
      role: node.role,
...
export function enhanceSurfaceMap(
  ...
  const midY = (b: BBox) => (b.top + b.bottom) / 2;
  const height = (b: BBox) => Math.max(0, b.bottom - b.top);
...
export function correlateIntegersWithAST(
  ...
  surfaceNumbers.forEach((sn, idx) => {
    console.log(
      `  [SURFACE-NUMS] [${idx}] surfaceId="${sn.id}" text="${
        sn.text || sn.latexFragment
      }" kind=${sn.kind} bbox=(${Math.round(sn.bbox.left)},${Math.round(
        sn.bbox.top
      )})`
    );
  });
  ...
  for (let i = 0; i < count; i++) {
    ...
    console.log(
      `[SURFACE-NUMS] MATCHED: surface[${i}] "${
        surfNum.text || surfNum.latexFragment
      }" (id=${surfNum.id}) -> AST nodeId="${astInt.nodeId}" value=${
        astInt.value
      } [DOM injected]`
    );
  }
...
export function correlateOperatorsWithAST(
  ...
  surfaceOperators.forEach((op, idx) => {
    console.log(
      `[SURFACE-OPS] index=${idx} op="${op.latexFragment}" surfaceId="${
        op.id
      }" bbox.left=${Math.round(op.bbox.left)} operatorIndex=${
        op.operatorIndex
      } astNodeId=${op.astNodeId || "NOT_SET"}`
    );
  });
  ...
  astOperators.forEach((op) => {
    ...
    console.log(
      `[DEBUG-NORM] AST: raw="${raw}" (U+${raw
        .charCodeAt(0)
        .toString(16)
        .toUpperCase()
        .padStart(4, "0")}) => normalized="${sym}"`
    );
  ...
  const surfaceBySymbol: Record<string, any[]> = {};
  surfaceOperators.forEach((op) => {
    const raw = op.latexFragment;
    const sym = normalizeOpSymbol(raw);
    console.log(
      `[DEBUG-NORM] Surface: raw="${raw}" (U+${raw
        .charCodeAt(0)
        .toString(16)
        .toUpperCase()
        .padStart(4, "0")}) => normalized="${sym}"`
  ...
  surfaceOperators.forEach((op, idx) => {
    ...
    console.log(
      `[SURFACE-OPS] index=${idx} op="${op.latexFragment}" surfaceId="${
        ...
        op.astNodeId || "UNMATCHED"
      }" astLocalIndex=${astLocal} globalIndex=${op.operatorIndex}`
    );
  });
 ...
 * @param {{root:any, atoms:any[], byElement:Map}} map - Surface map
 */
export function assertStableIdInjection(map: SurfaceNodeMap) {
  ...
  if (missing.length > 0) {
    ...
    console.error(
      `[STABLE-ID ASSERTION] ${
        missing.length
      } interactive element(s) missing data-ast-id: ${first3}${
        missing.length > 3 ? "..." : ""
      }`
    );
  } else {
