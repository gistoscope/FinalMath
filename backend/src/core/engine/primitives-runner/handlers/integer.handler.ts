...
export class IntegerPrimitiveHandler implements IPrimitiveHandler {
  ...
  handle(ctx: PrimitiveExecutionContext): AstNode | undefined {
    ...
    if (
      ...
      (primitiveId === "P.INT_DIV_TO_INT" || primitiveId === "P.INT_DIV_TO_FRAC")
    ) {
    ...
    if (targetNode.left.type !== "integer" || targetNode.right.type !== "integer") {
      return undefined;
    }
  ...
  private handleFractionAsDivision(ctx: PrimitiveExecutionContext): AstNode | undefined {
    const { root, targetNode, targetPath, primitiveId, astUtils } = ctx;
  ...
  private handleExactDivision(leftValue: string, rightValue: string): string | null {
