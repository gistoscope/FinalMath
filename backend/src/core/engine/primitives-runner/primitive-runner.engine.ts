...
import { AstParser, AstUtils } from "@/core/ast";
import { PrimitiveId } from "@/core/primitive-master";
import { injectable } from "tsyringe";

import { EngineStepExecutionRequest, EngineStepExecutionResult } from "../types.engine";
  ...
  run(req: EngineStepExecutionRequest): EngineStepExecutionResult {
    ...
    try {
      ...
      if (canUsePattern) {
        ...
        const ctx = this.contextFactory.createFromAst(ast, targetPath, primitiveId, {
          bindings,
          resultPattern,
        });
