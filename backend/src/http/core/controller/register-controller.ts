...
export function registerController(app: Express, controllers: Constructor[]) {
  controllers.forEach((Controller) => {
    ...
    console.log(`[registerController]: ${Controller.name} is registered `);
    const controllerMetadata: ControllerMetaData = {
      basePath: Reflect.getMetadata(CONTROLLER_KEY, Controller),
      routes: (Reflect.getMetadata(ROUTE_KEY, Controller.prototype) as RouteDefinition[]) || [],
      middlewares:
        (Reflect.getMetadata(CONTROLLER_MIDDLEWARE_KEY, Controller) as RequestHandler[]) || [],
    };
    ...
    if (controllerMetadata.middlewares.length > 0) {
      ...
      console.log(`[registerController]: middleware applied for controller ${Controller.name}`);
    }
    ...
    controllerMetadata.routes.forEach((route) => {
      ...
      const dto = Reflect.getMetadata(DTO_KEY, Controller.prototype, route.methodName);

      const handler = (controllerInstance as any)[route.methodName].bind(controllerInstance);

      const handleWithError = async (req: Request, res: Response, next: NextFunction) => {
        try {
      ...
      router[route.method](route.path, [...middlewares, ...dtoHandler, handleWithError]);
    });
