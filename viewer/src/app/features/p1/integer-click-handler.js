// features/p1/integer-click-handler.js
...
let _onHintApplySuccess = null;

/**
 * Set the callback for hint apply success
 * @param {Function} callback
 */
export function setOnHintApplySuccess(callback) {
  _onHintApplySuccess = callback;
}

/**
 * Handle client event for integer clicks
 * @param {object} ev - Client event
 */
export function handleClientEvent(ev) {
  if (
    ev &&
    ev.type === "click" &&
    ev.surfaceNodeKind &&
 ...
 */
export function handleIntegerClick(ev) {
  ...
  if (!effectiveAstId) {
    const surfaceNode = window.__currentSurfaceMap?.atoms?.find(
      (a) => a && a.id === surfaceId,
    );
  ...
  // CRITICAL FIX: Check if this is a non-targetable path
  const isNonTargetable =
    effectiveAstId &&
    effectiveAstId.startsWith &&
  ...
  // Handle double-click and single-click logic
  if (
    clickCount === 2 &&
 ...
 */
function handleDoubleClickBlueMode(clickStableKey) {
 ...
 * Handle double-click
 */
function handleDoubleClick(surfaceId, effectiveAstId, clickStableKey) {
  integerCycleState.dblclickLockUntil = Date.now() + 300;
...
/**
 * Process a confirmed single click (after double-click detection timeout)
 */
function processSingleClick(surfaceId, effectiveAstId, clickStableKey, astId) {
